...
public class SimpleSecurityManager extends SecurityManagerSupport {
	...
	private String toPermissionDescriptor(ResourcePermission permission) {
		StringBuilder builder = new StringBuilder(permission.getResource().getName());

		builder.append(":").append(permission.getOperation().getName());
	...
	/* (non-Javadoc) */
	private boolean isRequiredPermissionAttribute(String value) {
		return StringUtils.hasText(value) && !"*".equals(value.trim()); //{ it <= ' ' }
	}
}
